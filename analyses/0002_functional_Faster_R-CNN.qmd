...
class Plate_Image_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, Dict]:
        ...
        # Keep image as regular tensor initially
        target = {
...
This works! Quickly adding proper transforms.
...
```
This gave a type error, because the model is expecting float32 images in rang [0,1].
...
class Plate_Image_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, Dict]:
        # Load image and convert to float immediately
        img = read_image(self.img_files[idx])
        img = img.float() / 255.0  # Convert to float32 and scale to [0,1]
        ...
        # Apply only normalization transforms
        if self.transforms is not None:
            img = self.transforms(img)
...
```
Now trying to run the training script again with all these changes integrated together now:
```{python}
import caffeine 
...
epoch = train(model, train_loader, validation_loader, device, num_epochs, precedent_epoch, save_dir, optimizer)
```
These results will be stored in results/analysis_0002_02_15epochs.




To address this I need to modify both my model instance getter function and the plate image dataset class itself. \

Firstly, I will use PyTorch's [Compose class](https://pytorch.org/vision/main/generated/torchvision.transforms.Compose.html) to define the transforms to be applied to img:
