...
*Note:* This function uses train_one_epoch() from [engine.py](../src/torchvision_deps/engine.py) in torchvision_deps. I may later modify this function, and so I define it locally prior to these modifications before *my* train function to preserve its development through time.
```{python}
import math
import sys
import time
...
::: callout-note
I just learned about *args and **kwargs. I think this could help cut down the complexity of the function signatures. 
:::
...
[array([ 0.,  0.,  0., -1.,  0.,  0.,  0.,  0.,  0., -1.,  0.,  0.])] 
 ...
 -end-
 ```
This output is interesting. Firstly, the loss metrics are missing from eval_metrics, yet are output at the end of the previous section. This is because the evaluate_model() function returns only coco_evaluator's 'bbox' stats. I  will print coco_evaluator to see if the loss metrics are stored there:
```{python}
...
- Based on the print(eval_metrics) in the former code block, the array indexing in plot_eval_metrics needs to be adjusted (pertaining to the type error it currently produces). \
- Currently, I'm trying to plot the average loss and eval metrics. While this would be okay if I were training for many epochs, that won't work when trying to create a line plot for one epoch's training. Instead, I want to plot the metrics as the training or evaluation happens. Currently I am using train_one_epoch() and evaluate() from [engine](../src/torchvision_deps/engine.py) in torchvision_deps. They use PyTorch's [MetricLogger](https://pytorch.org/tnt/stable/utils/generated/torchtnt.utils.loggers.MetricLogger.html) to store metrics, where 'log_every()' from [coco_eval](../src/torchvision_deps/coco_eval.py) is handling the rate printing of metrics as training/evaluating progresses through the epoch. I want to plot the data from log_every()'s standard output. I suspect this will require modification to either log_every or the metric loggers in engine. This discussion is helpful: https://discuss.pytorch.org/t/object-detection-fine-tuning-interpreting-printout-of-train-one-epoch/193496/2 \
...
def train(model, data_loader, data_loader_test, device, num_epochs, precedent_epoch, save_dir, optimizer): 
    ...
    # train for one epoch, printing every 10 iterations
        ...
        for i, (k, v) in enumerate(metric_logger.meters.items()):
...
print("\n -end-")
```
The keys in metric_logger.meters are 'loss, loss_classifier, loss_box_reg, loss_objectness, loss_rpn_box_reg'. 'train_one_epoch' overwrites the meters value throughout training by making a call to MetricLogger.update(). I will modify 'train_one_epoch.py' to store intra-epoch loss metrics in a dictionary of lists (this requires additional defaultdict import) and their associated key (e.g., intra_epoch_loss['loss_classififer'] = []):
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq, scaler=None):
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # --insertion here-- #
        for i, (k, v) in enumerate(metric_logger.meters.items()):
            intra_epoch_log = defaultdict(list)
            intra_epoch_log = {'loss': [], 'loss_classifier': [], 'loss_box_reg': [], 'loss_objectness': [], 'loss_rpn_box_reg': []}
            if k in intra_epoch_log:
                intra_epoch_log[k].append(v)

        metric_logger.update(loss=losses_reduced, **loss_dict_reduced)
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])

    return metric_logger, intra_epoch_log
```
...
def train(model, data_loader, data_loader_test, device, num_epochs, precedent_epoch, save_dir, optimizer): 
    ...
    for epoch in range(num_epochs):
    # train for one epoch, printing every 10 iterations
        metric_logger, intra_epoch_losses = train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq=10)

        # plot loss throughout *this* epoch's training
        plot_training_loss(save_dir, epoch, [(k, v) for (k, v) in enumerate(intra_epoch_losses.items())])     # <----- insertion here
...
Now run the script, passing the project_dir_root created earlier instead of the save_dir to make sure that plots are saved in results/ which is now in the root of the project:
...
epoch, loss_metrics = train(model, data_loader, None, device, num_epochs, precedent_epoch, project_dir_root, optimizer)
