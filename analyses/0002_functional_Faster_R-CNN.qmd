...
- When defining plot_eval_metrics(), I drafted the code to plot the training loss metrics, but forgot I wanted to separate this into its own function. Currently train() makes a call to plot_eval_metrics. Instead I will define a plot_train_loss_metrics() function and make a call to that inside train() instead. \
...
I will define engine's evaluate and _get_iou_types functions here. **This has been modified** as both CocoEvaluator and Engine have a function called 'evaluate'. To avoid confusion, I have renamed Engine's evaluate function to 'evaluate_engine()':
```{python}
import math
import sys
import time
...
@torch.inference_mode()
def evaluate_engine(model, data_loader, device): # <------ modfification here
    n_threads = torch.get_num_threads()
    # FIXME remove this and make paste_masks_in_image run on the GPU
    torch.set_num_threads(1)
    cpu_device = torch.device("cpu")
...
def evaluate_model(model, data_loader, device):
    ...
    coco_evaluator = evaluate_engine(model, data_loader, device)
    print(coco_evaluator)
    # Extract evaluation metrics
    eval_stats = coco_evaluator.coco_eval['bbox'].stats
    val_metrics.append(eval_stats)
...
```{python}
@torch.inference_mode()
def evaluate_engine(model, data_loader, device):
    n_threads = torch.get_num_threads()
    # FIXME remove this and make paste_masks_in_image run on the GPU
    torch.set_num_threads(1)
    cpu_device = torch.device("cpu")
...
Now I will load a model saved from training earlier. Obviously, it has been trained for a very short period of time and won't yield any meaningful results. It will allow me to play around with engine and CocoEvaluator to expose the information I need for the plots. Firstly, I want to see if changing print frequency to 1 in evaluate() in engine prints each evaluation to std out.
...
This results in a print statement for each sample that lacks precision or recall metrics, only time metrics.:
```{python}
class CocoEvaluator:
    def __init__(self, coco_gt, iou_types):
        if not isinstance(iou_types, (list, tuple)):
        ...
        for iou_type, coco_eval in self.coco_eval.items():
            ...
            for img_id in coco_eval.params.imgIds:
                ...
                self.image_metrics[iou_type][img_id] = {
                    'precision': coco_eval.eval['precision'][:, :, :, img_id],
                    'recall': coco_eval.eval['recall'][:, img_id]
                }
