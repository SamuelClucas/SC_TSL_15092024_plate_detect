...
Now, I will inspect the CocoEvaluator constructor by adding some print statements:
```{python}
class CocoEvaluator:
    def __init__(self, coco_gt, iou_types):
        if not isinstance(iou_types, (list, tuple)):
...
Now I will load a model saved from training earlier. Obviously, it has been trained for a very short period of time and won't yield any meaningful results. It will allow me to play around with CocoEvaluator to expose the information I need for the plots.
