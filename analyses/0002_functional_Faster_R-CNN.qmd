...
Execution of the previous block of code produced an error pertaining to the plt_eval_metrics function in 'helper_training_functions'. Training seemed to complete 'successfully' (without any overt error message), as shown by the standard output:
```{bash}
#| code-fold: true
----- stdout -----
...
```{python}
model, optimizer, epoch = helper_training_functions.load_model('checkpoints/', 2, 'checkpoint_epoch_0')
...
def load_model(save_dir: str, num_classes: int, model_file_name: str):
    ...
    return model, optimizer, epoch
```
After reimporting the revised plate_detect.helper_training_functions with the above modifications (see imports for this sub-section), I ran the following script:
```{python}
device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
...
num_class = 2
_, _, preprocess = helper_training_functions.get_model_instance_object_detection(num_class) # load_model already calls model getter, no need to store it here
